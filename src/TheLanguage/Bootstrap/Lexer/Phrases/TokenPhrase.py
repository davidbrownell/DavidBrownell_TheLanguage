        ...
        async def LexAsync(
            ...
            with CallOnExit(lambda: observer.EndPhrase(unique_id, self, result is not None)):
                ...
                if isinstance(self.Token, IndentToken):
                    observer.OnPushScope(data, potential_iter_begin, potential_iter)
                elif isinstance(self.Token, DedentToken):
                    observer.OnPopScope(data, potential_iter_begin, potential_iter)
                elif not observer.OnInternalPhrase(data, potential_iter_begin, potential_iter):
                    return None
