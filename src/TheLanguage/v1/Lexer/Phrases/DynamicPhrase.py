...
class DynamicPhrase(Phrase):
    ...
    def _LexLeftRecursive(
        ...
        *,
        ...
        def PhraseIterator() -> Generator[Tuple[Phrase, Tuple[str, ...]], None, None]:
            ...
            while True:
                yield (suffix_phrase, unique_id + ("__Suffix__", str(iterator), ))
                iterator += 1
        ...
        if not results:
            return Phrase.LexResult.Create(
                ...
                Phrase.LexResultData.Create(self, unique_id, error_result, None),
            )
        ...
        for result in results[1:]:
            result_data = self.__class__.GetDynamicData(result)
            ...
            if self.__class__.IsRightRecursivePhrase(result_data.phrase):
                right = result_data.data[-1]
                assert isinstance(right, Phrase.LexResultData)

                right_data = self.__class__.GetDynamicData(right)
                ...
                while True:
                    ...
                    if not self.__class__.IsRightRecursivePhrase(potential_leftmost_right_data.phrase):
                        break
                    ...
                    potential_leftmost_right_data = self.__class__.GetDynamicData(potential_leftmost_right)
        ...
        if not self._is_valid_data_func(root):
            return Phrase.LexResult.Create(
                False,
                Phrase.NormalizedIteratorRange.Create(starting_iter, normalized_iter),
                root,
    ...
    class _SequenceSuffixWrapper(Phrase):
        ...
        def __init__(
            ...
            super(DynamicPhrase._SequenceSuffixWrapper, self).__init__("Wrapper ({})".format(phrase.name))  # pylint: disable=protected-access
