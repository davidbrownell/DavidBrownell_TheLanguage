...
class LeftRecursiveSequencePhraseWrapper(Phrase):
    ...
    async def _LexAsyncImpl(
        ...
        result = await self._prefix_phrase.LexAsync(
            unique_id + ("Prefix", ),
            normalized_iter,
            observer,
        ...
        if result.Success:
            ...
            while True:
                result = await self._suffix_phrase.LexAsync(
        ...
        for data_item_index, data_item in enumerate(data_items):
            ...
            if data_item_index != len(data_items) - 1:
                ...
                assert isinstance(data_item_next.Data.Data, Phrase.MultipleStandardLexResultData)
...
class _SuffixWrapper(Phrase):
    ...
    async def _LexAsyncImpl(
        ...
        return await cast(SequencePhrase, self._phrase).LexSuffixAsync(
            unique_id,
            normalized_iter,
            observer,
            ignore_whitespace=ignore_whitespace,
