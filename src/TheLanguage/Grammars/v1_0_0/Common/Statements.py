...
def ExtractDynamicExpressionsNode(
    ...
    """Drill down to the specific content when given a node generated by matching `DynamicStatements.Expressions`"""

    # Drill past the Dynamic Expression node
    assert node.Type == DynamicStatements.Expressions
    assert len(node.Children) == 1
    node = node.Children[0]

    # Drill past the grammar node
    assert len(node.Children) == 1
    node = node.Children[0]
